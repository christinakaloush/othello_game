...
def main():
    ...
    while not game.is_game_over():
        ...
        if not game.is_a_valid_move_available(current_player_disc):
            ...
            while True:
                data = input(f" {current_player}'s Turn ({disc_colors[current_player_disc]}) - Where do you like to place the disc (row# col#)? ")
                row, col = data.split()
                row = int(row)
                col = int(col)
    ...
    if winner == Othello.TIE:
        ...
        print(f"Game over. The winner is player {winner} ({disc_colors[current_player_disc]})")
